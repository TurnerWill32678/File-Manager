...
class App : Application(), coil3.SingletonImageLoader.Factory {
    ...
    val appFiles: LocalFileHolder
        get() = LocalFileHolder(
            File(Environment.getExternalStorageDirectory(), ".fileManager")
    ...
    override fun onCreate() {
        ...
        // Anonymous authentication (silent, no UI)
        FirebaseAuth.getInstance().signInAnonymously()
            .addOnSuccessListener {
                Log.d("MyApp", "Firebase anonymous auth success: ${it.user?.uid}")
            }

            .addOnFailureListener { e ->
                Log.e("MyApp", "Firebase auth failed", e)
            }
